...
export class HttpRequestSanitizerInterceptor implements NestInterceptor {
  ...
  cleanRequest(request: Request): void {
    (request.query as any) = this.cleanObject(request.query);
    (request.params as any) = this.cleanObject(request.params);
