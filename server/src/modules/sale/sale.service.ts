...
export class SaleService {
  ...
  public async openTransaction(
    ...
    const transactionHeader =
      await this.fetchFinishedTransactionHeader(transactionId);

    if (!transactionHeader) {
      throw responseMessages.TRANSACTION_ID_NOT_FOUND;
    }
  ...
  public async updateCart(
    ...
    if (isValidPrice === false) {
      throw responseMessages.INVALID_PRICE;
    }
    ...
    return responseMessages.ADDED_TO_CART;
  }
  ...
  public async emptyCart(transactionId: number): Promise<any> {
    const transactionHeader =
      await this.fetchInProcessTransactionHeader(transactionId);
    ...
    return responseMessages.CART_EMPTIED;
  }
  ...
  private async deleteCreditSale(
    ...
    try {
      ...
      if (!previousTransaction || !pointer) {
        throw responseMessages.NO_DATA_FOUND;
      }
      ...
      return responseMessages.SALE_DELETED;
    } catch (error) {
      queryRunner.rollbackTransaction();
      throw error;
    }
  ...
  private async saveCreditSale(
    ...
    if (count !== 1) {
      throw responseMessages.INVALID_CUSTOMER;
    }
  ...
  private async verifyTheBalanceIsLatest(
    ...
    if (count !== 1) {
      throw responseMessages.BALANCE_MISMATCH;
    }
    return true;
  }
